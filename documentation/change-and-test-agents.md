...
| Test Author | Turns compliance intel or a named suite into tests. |
| Test | Triggers the run, records risk, writes the timestamped result. |

## What a successful change looks like

Apply on twin IOS-XE → Test → approve → prod. Design follows the same
gate: size on the twin, test, then ServiceNow logistics.

Test writes `testing/<stamp>.json` and `state/testing.json` after
...
score is Test, not the intel scan.
